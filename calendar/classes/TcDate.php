...
class TcDate extends TcDateMain{
	...
	public function getDayOfWeek($cdate = ""){
		if(!$this->compatible){
			return parent::getDayOfWeek($cdate);
		}else{
	...
	public function getWeekNumber($cdate = ""){
		if(!$this->compatible){
			return parent::getWeekNumber($cdate);
		}else{
			if(($cdate != "" && $this->validDate($cdate)) || $cdate == ""){
				$tmp_date = ($cdate != "") ? new DateTime($cdate) : $this->mydate;
				return $tmp_date->format('W');
	...
	public function setDate($sdate){
		if(!$this->compatible){
			parent::setDate($sdate);
		}else{
			if(parent::validDate($sdate))
	...
	public function getDate($format = "Y-m-d", $cdate = ""){
		if(!$this->compatible){
			return parent::getDate($format, $cdate);
		}else{
			if(($cdate != "" && $this->validDate($cdate)) || $cdate == ""){
				$tmp_date = ($cdate != "") ? new DateTime($cdate) : $this->mydate;
	...
	public function getTimestamp($cdate = ""){
		if(!$this->compatible){
			return parent::getTimestamp($cdate);
		}else{
			if(($cdate != "" && $this->validDate($cdate)) || $cdate == ""){
				$tmp_date = ($cdate != "") ? new DateTime($cdate) : $this->mydate;
	...
	public function getDateFromTimestamp($stime, $format = 'Y-m-d'){
		if($stime){
			if(!$this->compatible){
				return parent::getDateFromTimestamp($stime, $format);
			}else{
	...
	public function differentDate($date2, $date1 = ""){
		if(!$this->compatible){
			return parent::differentDate($date2, $date1);
		}else{
	...
	public function dateBefore($date2, $date1 = "", $equal = true){
		if(!$this->compatible){
			return parent::dateBefore($date2, $date1, $equal);
		}else{
			$date1 = ($date1 != "") ? $date1 : $this->getDate('Y-m-d');
	...
	public function dateAfter($date2, $date1 = "", $equal = true){
		if(!$this->compatible){
			return parent::dateAfter($date2, $date1, $equal);
		}else{
			$date1 = ($date1 != "") ? $date1 : $this->getDate('Y-m-d');

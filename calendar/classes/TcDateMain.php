...
class TcDateMain{
	...
	public function addMonth($format = "Y-m-d", $timespan = 0, $cdate = ""){
		if(($cdate != "" && $this->validDate($cdate)) || $cdate == ""){
			...
			return date($format, mktime(0,0,0,(date('m', $tmp_date)+$timespan),
			    date('d', $tmp_date),date('Y', $tmp_date)));
		}else return "0000-00-00";
	}
	...
	public function addYear($format = "Y-m-d", $timespan = 0, $cdate = ""){
		if(($cdate != "" && $this->validDate($cdate)) || $cdate == ""){
			...
			return date($format, mktime(0,0,0,date('m', $tmp_date),
			    date('d', $tmp_date),(date('Y', $tmp_date)+$timespan)));
		}else return "0000-00-00";
	}
	...
	public function validDate($date_str){
		if($date_str != ""){
			...
			if((isset($date_arr[0]) && is_numeric($date_arr[0])) && (isset($date_arr[1]) && 
			    is_numeric($date_arr[1])) && (isset($date_arr[2]) && is_numeric($date_arr[2]))){
